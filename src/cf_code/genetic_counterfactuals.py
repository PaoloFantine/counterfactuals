...
class GA_counterfactuals:
    ...
    def generate_counterfactuals(
        ...
        while (
            len(solutions) < 2 * n_counterfactuals
        ):  # finding more counterfactuals than needed to allow for selection of the fittest ones
            # -- evaluate parents fitness
            if self.problem_type == "classification":
                parents["outcome"] = self.model.predict_proba(parents[self.X.columns])[
                    :, desired_class
                ]
                ...
                selected_parents = parents[
                    (parents["outcome"] >= lower_limit)
                    & (parents["outcome"] <= upper_limit)
                ]
            ...
            if not selected_parents.empty:
                if solutions.empty:
                    ...
                    solutions = pd.concat(
                        [solutions, selected_parents], ignore_index=True
                    )
            ...
            for idx1, idx2 in couples:
                ...
                for col in self.X.columns:
                    if (
                        parent1[col].dtype in ["int64", "int32", "object", "category"]
                        and col not in self.one_hot_encoded
                    ):
                        # for categorical or int values, choose the value of one of the parents
                        # with equal probability
                        child[col] = npr.choice(
                            [parent1[col].values[0], parent2[col].values[0]],
                            1,
                            p=[0.5, 0.5],
                        )
                ...
                if self._ohe_prefixes:
                    for prefix in self._ohe_prefixes:
                        ohe_parent = npr.choice(
                            ["parent1", "parent2"], 1, p=[0.5, 0.5]
                        )[0]
                        ohe_cols = [
                            col for col in self.X.columns if col.startswith(prefix)
                        ]
                        for col in ohe_cols:
            ...
            if count % 10 == 0:
                print(
                    f"Generation {count}; counterfactuals found: {counterfactuals_found}/{n_counterfactuals}"
                )
                if (
                    counterfactuals_found == n_counterfactuals
                ):  # if there are enough counterfactuals after 10+ generations, stop looking
                    break
    ...
    def _initialize_parents(self, base, n_counterfactuals):
        ...
        mutation_batch = pd.concat(
            [
                self._data_mutation(
                    to_be_mutated=base, features_to_vary=self.features_to_vary
                )
                for _ in range(2 * n_counterfactuals)
            ]
        )
    ...
    def _data_mutation(self, to_be_mutated, features_to_vary):
        ...
        regular_features = [
            col for col in features_to_vary if col not in self.one_hot_encoded
        ]
        ...
        for col in regular_features:
            ...
            gene_dict[col] = npr.choice(
                [mutation[0], gene_dict[col][0]], 1, p=[0.2, 0.8]
            )
    ...
    def _evaluate_fitness(
        self, current_generation, desired_class=None, lower_limit=None, upper_limit=None
    ):
        """
        ...
        current_generation["outcome_fitness"] = self._solution_fitness(
            current_generation, lower_limit=lower_limit, upper_limit=upper_limit
        )

        # point-likelihood
        # -- check how likely it is for the point to be in the feature space
        current_generation["point_likelihood_fitness"] = self._point_likelihood_fitness(
            current_generation
        )
        ...
        current_generation["sparsity_fitness"] = len(
            self.X.columns
        ) - current_generation[self.X.columns].apply(
            lambda x: list(
                np.not_equal(x.values, self.base[self.X.columns].values)[0]
            ).count(True),
            axis=1,
        )
        ...
        base_current = pd.concat(
            [self.base, current_generation[self.X.columns]], axis=0, ignore_index=True
        )
        ...
        current_generation["closeness_fitness"] = (
            1 - gower.gower_matrix(base_current[self.X.columns])[0, 1:].mean()
        )

        return current_generation.reset_index(drop=True)

    def _solution_fitness(self, current_generation, lower_limit=None, upper_limit=None):
        """
        ...
        if self.problem_type == "classification":
            ...
            outcome_fitness = np.minimum(
                abs(current_generation["outcome"] - lower_limit),
                abs(upper_limit - current_generation["outcome"]),
            )
    ...
    def _point_likelihood_fitness(self, current_generation):
        ...
        point_distribution_dfs = [
            pd.concat(
                [
                    current_generation[self.X.columns].iloc[i : i + 1],
                    nearest_neighbors[i],
                ],
                axis=0,
                ignore_index=True,
        ...
        for i in range(len(point_distribution_dfs)):
            for col in (
                point_distribution_dfs[i].select_dtypes(include=["category"]).columns
            ):
                point_distribution_dfs[i][col] = point_distribution_dfs[i][col].astype(
                    "object"
                )
        ...
        gower_distance = [
            1 - gower.gower_matrix(point_distribution_dfs[i])[0, 1:].mean()
            for i in range(len(point_distribution_dfs))
        ]
    ...
    def _nondominance_sorting(self, current_fitness):
        ...
        for i in current_fitness.index:
            for j in current_fitness.index:
                ...
                if (
                    list(current_fitness.iloc[i] > current_fitness.iloc[j]).count(True)
                    > 0
                ) and (
                    list(current_fitness.iloc[i] < current_fitness.iloc[j]).count(True)
                    == 0
        ...
        while dominates:
            ...
            front_idx = [
                i for i in dominated_by_count.keys() if dominated_by_count[i] == 0
            ]
            ...
            reduce_dominated_by_count_flattened = [
                item for sublist in reduce_dominated_by_count for item in sublist
            ]
    ...
    def _evaluate_selection_probability(
        self, parents, desired_class=None, lower_limit=None, upper_limit=None
    ):
        """
        ...
        current_generation["front"] = self._nondominance_sorting(
            current_generation[
                [
                    "outcome_fitness",
                    "point_likelihood_fitness",
                    "sparsity_fitness",
                    "closeness_fitness",
                ]
            ].copy()
        ...
        # rank fronts by
        current_generation["front_rank"] = current_generation["front"].rank(
        ...
        current_generation["selection_probability"] = current_generation[
            "combined_rank"
        ] / sum(current_generation["combined_rank"])
