"""
Train an XGBoost regressor on the California housing dataset,
save the model and a classification report to `artifacts/`.
Run via:
    python scripts/regression_training.py --test-size 0.1 --random-state 42
"""

import argparse
import json
import logging
from pathlib import Path

import joblib
from sklearn.metrics import mean_absolute_error, mean_squared_error, r2_score
from sklearn.model_selection import train_test_split
from xgboost import XGBRegressor

from src.data_loader import load_dataset

ARTIFACT_DIR = Path("artifacts")
ARTIFACT_DIR.mkdir(exist_ok=True)

logging.basicConfig(
    level=logging.INFO, format="%(asctime)s  %(levelname)s  %(message)s"
)


def train(test_size: float, random_state: int) -> None:
    # Load data and preprocess it
    logging.info("Loading dataset …")
    X, y = load_dataset(
        "california_housing", preprocess=True
    )  # assuming regression dataset

    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=test_size, random_state=random_state
    )

    # Train model
    logging.info("Training XGBoost Regressor …")
    reg = XGBRegressor(
        objective="reg:squarederror",
        random_state=random_state,
    )
    reg.fit(X_train, y_train)

    # Evaluate model
    logging.info("Evaluating …")
    y_pred = reg.predict(X_test)
    metrics = {
        "mse": mean_squared_error(y_test, y_pred),
        "mae": mean_absolute_error(y_test, y_pred),
        "r2": r2_score(y_test, y_pred),
    }

    # store evaluation metrics
    logging.info("Generating regression report …")
    with open(ARTIFACT_DIR / "regression_report.json", "w") as f:
        json.dump(metrics, f, indent=2)
    logging.info("Saved regression_report.json")

    # Serialize model
    joblib.dump(reg, ARTIFACT_DIR / "xgboost_regressor.pkl")
    logging.info("Saved xgboost_regressor.pkl")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser()
    p.add_argument("--test-size", type=float, default=0.10)
    p.add_argument("--random-state", type=int, default=42)
    return p.parse_args()


if __name__ == "__main__":
    args = parse_args()
    train(args.test_size, args.random_state)
